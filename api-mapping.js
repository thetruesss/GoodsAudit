...
(function (root, factory) {
  ...
  function createRequestPacer(rps, opts = {}) {
    ...
    return {
      ...
      // Различаем «сервис просит сбавить» и «у нас не получилось отправить» —
      // это разные вещи, и реакция на них разная.
      report({ status = 0, ms = 0, retryAfterMs = 0 } = {}) {
        if (st.rate <= 0) return;
        // Ноль — это «запрос не ушёл»: наша вкладка была занята переходом, связь
        // моргнула. Сервис тут ни при чём, ронять из-за этого скорость вдвое
        // неправильно — но и разгоняться дальше рано, пока не проехали чисто.
        if (status === 0) {
          st.okStreak = 0;
          return;
        }
        const pushback = status === 429 || (status >= 500 && status <= 599);
        if (pushback) {
          st.throttles += 1;
          st.okStreak = 0;
