(function () {
  ...
  // --- DEV-снимок страницы ---------------------------------------------------
  // Отдаёт не только готовый снапшот, но и то, из чего он собран: таблицы с
  // «Номенклатурой», подписи свойств, бейджи. Нужен, чтобы разбирать
  // расхождения с API не по одной строке диагностики, а по факту вёрстки.
  function devSnapshotOfPage(doc, opsWarehouses) {
    const cut = (s, n) => {
      const t = String(s ?? "")
        .replace(/\u00a0/g, " ")
        .replace(/\s+/g, " ")
    ...
    for (const table of doc.querySelectorAll("table")) {
      const headers = [...table.querySelectorAll("thead th")].map((th) => cut(th.textContent, 60));
      if (!headers.some((h) => h.includes(LBL_NOM))) continue;
      const rows = [...table.querySelectorAll("tbody tr")]
        .slice(0, 6)
        .map((tr) => [...tr.querySelectorAll("td")].map((td) => cut(td.innerText || td.textContent, 90)));
      tables.push({ headers, rows, totalRows: table.querySelectorAll("tbody tr").length });
    }

    // Все подписи сетки свойств: видно, какие поля вообще есть на карточке.
    const grid = [];
    for (const el of doc.querySelectorAll("div, span, p")) {
      const label = cut(el.textContent, 40);
      if (!label || label.length > 40) continue;
      if (el.children.length) continue;
      const row = el.closest("[class*='propertiesGridRow'], [class*='_propertiesGridRow']");
      if (!row) continue;
      const content =
        row.querySelector("[class*='_content'], [class*='content']") || el.nextElementSibling;
      if (!content || content === el) continue;
      const value = cut(content.innerText || content.textContent, 120);
      if (!value || value === label) continue;
      if (grid.some((g) => g.label === label && g.value === value)) continue;
      grid.push({ label, value });
      if (grid.length >= 40) break;
    }

    const badges = [...doc.querySelectorAll("[class*='badge']")]
      .slice(0, 12)
      .map((b) => ({
        title: cut(b.getAttribute("title") || "", 60),
        text: cut(b.textContent, 60),
      }))
      .filter((b) => b.title || b.text);

    const bodyText = String(doc.body?.innerText || "");
    return {
      snapshot: extractFromDocument(doc, opsWarehouses),
      nomenclatureTables: tables,
      gridProperties: grid,
      badges,
      markers: {
        hasTransitBoxNumber: bodyText.includes("\u041d\u043e\u043c\u0435\u0440 \u0442\u0440\u0430\u043d\u0437\u0438\u0442\u043d\u043e\u0439 \u043a\u043e\u0440\u043e\u0431\u043a\u0438"),
        hasTransitBoxWord: bodyText.includes("\u0422\u0440\u0430\u043d\u0437\u0438\u0442\u043d\u0430\u044f \u043a\u043e\u0440\u043e\u0431\u043a\u0430"),
        hasUnsupported: textHasUnsupportedItemMarker(bodyText),
  ...
  globalThis.__returnsDevProbe = function (opsWarehouses) {
    try {
      return devSnapshotOfPage(document, Array.isArray(opsWarehouses) ? opsWarehouses : []);
    } catch (e) {
